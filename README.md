...
- Разностных обновлений нет: пока не видно, что они окупят сложность — сборка и так
  докачивает только изменившиеся файлы.
- На macOS нет аппаратного ключа: Secure Enclave требует биндингов к Security.framework.
- Ключ из TPM проверен на живом Linux. Ветка для Windows написана, но на железе не проверялась.
- Сборки дублируют ассеты и ванильный jar на диске сервера — общий слой не построен.
- У `scrub_cas` нет точки входа в интерфейсе.
